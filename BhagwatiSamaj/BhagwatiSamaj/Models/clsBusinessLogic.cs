...
" WHERE Id=" + matrimonial.Id);
        ...
        public static List<YagnaMember> GetYagnaMembers()
        {
            return SqlHelper.ConvertToEnumerable<YagnaMember>(SqlHelper.GetDataTable("select * from YagnaMember")).ToList();
        }

        public static void DeleteYagnaMember(string id)
        {
            SqlHelper.ExecuteNonQuery("delete from YagnaMember where Id=" + id);
        }

        public static int SaveYagnaMember(YagnaMember member)
        {
            int memberId = 0;
            if (member.Id > 0)
            {
                SqlHelper.ExecuteNonQuery("Update YagnaMember set Name='" + member.Name + "',Address='" +
                    member.Address + "',Mobile='" + member.Mobile + "' where Id=" + member.Id);
            }
            else
            {
                memberId = Convert.ToInt32(SqlHelper.ExecuteScalar("Insert into YagnaMember(Name,Address,Mobile) values('" +
                    member.Name + "','" + member.Address + "','" + member.Mobile + "'); select @@IDENTITY"));
            }
            return memberId;
        }
        ...
        internal static string PrintMemberReport()
        {
            ...
            float[] columnWidths = new float[3] { columnWidth, columnWidth, columnWidth };
