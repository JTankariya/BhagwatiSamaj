...
namespace BhagwatiSamaj.Controllers
{
    public class AdminController : Controller
    {
        ...
        public ActionResult YagnaMemberList()
        {
            var list = clsBusinessLogic.GetYagnaMembers();
            return View(list);
        }
        ...
        public ActionResult EditYagnaMember(string Id)
        {
            if (Id == "0")
            {
                var envt = new YagnaMember();
                return View(envt);
            }
            else
            {
                var evnt = clsBusinessLogic.GetYagnaMembers().FirstOrDefault(x => x.Id == Convert.ToInt32(Id));
                return View(evnt);
            }
        }
        ...
        public ActionResult DeleteYagnaMember(string id)
        {
            ResponseMsg response = new ResponseMsg();
            try
            {
                clsBusinessLogic.DeleteYagnaMember(id);
                response.IsSuccess = true;
                response.ResponseValue = "";
            }
            catch (Exception ex)
